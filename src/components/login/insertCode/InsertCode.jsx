    ...
    <div className='formphone m-5'>
      ...
      <GiStrawberry className='drop-shadow-2xl text-shadow text-blue-400 text-5xl self-center'/>
        <label htmlFor="" className='flex flex-col gap-3'>
          ...
          <input {...register('code')} className='border-2 border-blue-200 p-2 rounded-md' type="text" placeholder='código sms' />
        </label>
        {errors.code ? <span className='bg-red-200 p-2 text-white'>{errors.code.message}</span> : <></>}
        <button type='submit' className='bg-blue-300 p-3 rounded-md'>Verificar código</button>
      </form>
    </div>
  )
}
